...
String ls_sender, ls_recipients[], ls_charset, ls_errmsg
...
If sle_sender_email.text = "" Then
	...
	Return
End If
If Not ln_smtp.ValidEmail(sle_sender_email.text, ls_errmsg) Then
	sle_sender_email.SetFocus()
...
ls_sender    = ln_smtp.EmailName(sle_sender_email.text, sle_sender_name.text)
lb_Priority  = cbx_priority.Checked

// get recipients
If sle_recip_email1.text <> "" Then
	If Not ln_smtp.ValidEmail(sle_recip_email1.text, ls_errmsg) Then
		sle_recip_email1.SetFocus()
		MessageBox("To Email #1 Format Error", ls_errmsg, StopSign!)
		Return
	End If
	li_max = UpperBound(ls_recipients) + 1
	ls_recipients[li_max] = sle_recip_email1.text
End If
If sle_recip_email2.text <> "" Then
	If Not ln_smtp.ValidEmail(sle_recip_email2.text, ls_errmsg) Then
		sle_recip_email1.SetFocus()
		MessageBox("To Email #2 Format Error", ls_errmsg, StopSign!)
		Return
	End If
	li_max = UpperBound(ls_recipients) + 1
	ls_recipients[li_max] = sle_recip_email2.text
End If
If sle_recip_email3.text <> "" Then
	If Not ln_smtp.ValidEmail(sle_recip_email3.text, ls_errmsg) Then
		sle_recip_email1.SetFocus()
		MessageBox("To Email #3 Format Error", ls_errmsg, StopSign!)
		Return
	End If
	li_max = UpperBound(ls_recipients) + 1
	ls_recipients[li_max] = sle_recip_email3.text
End If
If sle_recip_email4.text <> "" Then
	If Not ln_smtp.ValidEmail(sle_recip_email4.text, ls_errmsg) Then
		sle_recip_email1.SetFocus()
		MessageBox("To Email #4 Format Error", ls_errmsg, StopSign!)
		Return
	End If
	li_max = UpperBound(ls_recipients) + 1
	ls_recipients[li_max] = sle_recip_email4.text
End If
...
try
	...
	If ls_userid <> "" Then
		ln_smtp.SetUserNamePassword(ls_userid, ls_passwd)
	End If
	ln_smtp.SetPort(li_port)
	ln_smtp.SetAuthMethod(li_AuthMethod)
	ln_smtp.SetConnectionType(li_ConnType)
	ln_smtp.SetCharSet(ls_Charset)
	If lb_Priority Then
		ln_smtp.SetPriority(ln_smtp.HighPriority)
	End If
	// connect to the server
	li_rc = ln_smtp.SmtpConnect()
...
For li_idx = 1 To li_max
	try
		...
		// Set Custom Header (only one allowed)
		ln_smtp.SetHeader("Message-ID", "PowerBuilder-" + String(Rand(32767)))
