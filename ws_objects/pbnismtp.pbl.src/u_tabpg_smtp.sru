...
String ls_sender, ls_recipient, ls_ccrecip, ls_charset, ls_errmsg, ls_work
...
If sle_recip_email.text = "" Then
	...
	Return
End If
If Not ln_smtp.ValidEmail(sle_recip_email.text, ls_errmsg) Then
	sle_recip_email.SetFocus()
...
ls_subject   = 'Recuperação de Senha'
ls_recipient = ln_smtp.EmailName(sle_recip_email.text, sle_recip_name.text)
ls_sender    = ln_smtp.EmailName(sle_sender_email.text, sle_sender_name.text)
ls_ccrecip   = ln_smtp.EmailName(sle_cc_email.text, sle_cc_name.text)
...
try
	...
	If ls_ccrecip <> "" Then
		ln_smtp.SetCCRecipientEmail(ls_ccrecip)
	End If
	ln_smtp.SetSubject(ls_subject)
	ln_smtp.SetMessage(ls_body, lb_Html)
	ln_smtp.SetReadReceiptRequested(lb_Receipt)
	If lb_Priority Then
	...
	If li_rc = 1 Then
		MessageBox("Sucesso!", "O Email de recuperação foi enviado")
	Else
